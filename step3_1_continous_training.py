...
def reduce_sum(metric: torch.tensor):
    """
    将多卡计算的指标加起来
    """
...
def main_worker(rank, world_size):
    ...
    torch.cuda.set_device(rank) # assign which gpu will be used (only linux works)
    ...
    previous_check_point_path = './models/8/'
    previous_check_point_name = 'latest_checkpoint.tar'
    
    model_path = './models/8/'
    model_name = 'MeshSegNet_15_classes' #remember to include the project title (e.g., ALV)
    ...
    start_times, end_times, val_start_times, val_end_times = [], [], [], []
    ...
    for epoch in range(epoch_init, num_epochs):
        ...
        for i_batch, batched_sample in enumerate(train_loader):
            ...
            if i_batch % num_batches_to_print == num_batches_to_print - 1:
                if use_visdom:
                    plotter.plot(f'loss {rank}', 'train', 'Loss', epoch + (i_batch + 1) / len(train_loader),
                                 running_loss / num_batches_to_print)
                    plotter.plot(f'DSC {rank}', 'train', 'DSC', epoch + (i_batch + 1) / len(train_loader),
                                 running_mdsc / num_batches_to_print)
                    plotter.plot(f'SEN {rank}', 'train', 'SEN', epoch + (i_batch + 1) / len(train_loader),
                                 running_msen / num_batches_to_print)
                    plotter.plot(f'PPV {rank}', 'train', 'PPV', epoch + (i_batch + 1) / len(train_loader),
                                 running_mppv / num_batches_to_print)

                # 统计多卡的指标
                loss_mean = reduce_sum(running_loss) / (num_batches_to_print * world_size)
                mdsc_mean = reduce_sum(running_mdsc) / (num_batches_to_print * world_size)
                msen_mean = reduce_sum(running_msen) / (num_batches_to_print * world_size)
                mppv_mean = reduce_sum(running_mppv) / (num_batches_to_print * world_size)
                if rank == 0:
        ...
        losses.append(reduce_sum(loss_epoch) / len(train_loader) / world_size)
        mdsc.append(reduce_sum(mdsc_epoch) / len(train_loader) / world_size)
        msen.append(reduce_sum(msen_epoch) / len(train_loader) / world_size)
        mppv.append(reduce_sum(mppv_epoch) / len(train_loader) / world_size)
        ...
        with torch.no_grad():
            ...
            for i_batch, batched_val_sample in enumerate(val_loader):
                ...
                if i_batch % num_batches_to_print == num_batches_to_print - 1:
                    # 统计多张卡的指标
                    val_loss_mean = reduce_sum(running_val_loss) / (num_batches_to_print * world_size)
                    val_mdsc_mean = reduce_sum(running_val_mdsc) / (num_batches_to_print * world_size)
                    val_msen_mean = reduce_sum(running_val_msen) / (num_batches_to_print * world_size)
                    val_mppv_mean = reduce_sum(running_val_mppv) / (num_batches_to_print * world_size)
                    if rank == 0:
            ...
            val_losses.append(reduce_sum(val_loss_epoch) / len(val_loader) / world_size)
            val_mdsc.append(reduce_sum(val_mdsc_epoch) / len(val_loader) / world_size)
            val_msen.append(reduce_sum(val_msen_epoch) / len(val_loader) / world_size)
            val_mppv.append(reduce_sum(val_mppv_epoch) / len(val_loader) / world_size)
