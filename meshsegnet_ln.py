...
class MeshSegNet(nn.Module):
    def __init__(self, num_classes=15, num_channels=15, with_dropout=True, dropout_p=0.5, cells=5000, feature_base=64):
        ...
        assert feature_base % 2 == 0

        # MLP-1 [64, 64]
        self.mlp1_conv1 = torch.nn.Conv1d(self.num_channels, feature_base, 1)
        self.mlp1_conv2 = torch.nn.Conv1d(feature_base, feature_base, 1)
        # self.mlp1_bn1 = nn.BatchNorm1d(64)
        self.mlp1_ln1 = nn.LayerNorm([feature_base, cells])
        # self.mlp1_bn2 = nn.BatchNorm1d(64)
        self.mlp1_ln2 = nn.LayerNorm([feature_base, cells])
        # FTM (feature-transformer module)
        self.fstn = STNkd(k=feature_base, cells=cells)
        # GLM-1 (graph-contrained learning modulus)
        self.glm1_conv1_1 = torch.nn.Conv1d(feature_base, feature_base // 2, 1)
        self.glm1_conv1_2 = torch.nn.Conv1d(feature_base, feature_base // 2, 1)
        # self.glm1_bn1_1 = nn.BatchNorm1d(32)
        self.glm1_ln1_1 = nn.LayerNorm([feature_base // 2, cells])
        # self.glm1_bn1_2 = nn.BatchNorm1d(32)
        self.glm1_ln1_2 = nn.LayerNorm([feature_base // 2, cells])
        self.glm1_conv2 = torch.nn.Conv1d(feature_base, feature_base, 1)
        # self.glm1_bn2 = nn.BatchNorm1d(64)
        self.glm1_ln2 = nn.LayerNorm([feature_base, cells])
        # MLP-2
        self.mlp2_conv1 = torch.nn.Conv1d(feature_base, feature_base, 1)
        # self.mlp2_bn1 = nn.BatchNorm1d(64)
        self.mlp2_ln1 = nn.LayerNorm([feature_base, cells])
        self.mlp2_conv2 = torch.nn.Conv1d(feature_base, feature_base * 2, 1)
        # self.mlp2_bn2 = nn.BatchNorm1d(128)
        self.mlp2_ln2 = nn.LayerNorm([feature_base * 2, cells])
        self.mlp2_conv3 = torch.nn.Conv1d(feature_base * 2, feature_base * 8, 1)
        # self.mlp2_bn3 = nn.BatchNorm1d(512)
        self.mlp2_ln3 = nn.LayerNorm([feature_base * 8, cells])
        # GLM-2 (graph-contrained learning modulus)
        self.glm2_conv1_1 = torch.nn.Conv1d(feature_base * 8, feature_base * 2, 1)
        self.glm2_conv1_2 = torch.nn.Conv1d(feature_base * 8, feature_base * 2, 1)
        self.glm2_conv1_3 = torch.nn.Conv1d(feature_base * 8, feature_base * 2, 1)
        # self.glm2_bn1_1 = nn.BatchNorm1d(128)
        self.glm2_ln1_1 = nn.LayerNorm([feature_base * 2, cells])
        # self.glm2_bn1_2 = nn.BatchNorm1d(128)
        self.glm2_ln1_2 = nn.LayerNorm([feature_base * 2, cells])
        # self.glm2_bn1_3 = nn.BatchNorm1d(128)
        self.glm2_ln1_3 = nn.LayerNorm([feature_base * 2, cells])
        self.glm2_conv2 = torch.nn.Conv1d(feature_base * 6, feature_base * 8, 1)
        # self.glm2_bn2 = nn.BatchNorm1d(512)
        self.glm2_ln2 = nn.LayerNorm([feature_base * 8, cells])
        # MLP-3
        self.mlp3_conv1 = torch.nn.Conv1d(feature_base * 25, feature_base * 4, 1)
        self.mlp3_conv2 = torch.nn.Conv1d(feature_base * 4, feature_base * 4, 1)
        # self.mlp3_bn1_1 = nn.BatchNorm1d(256)
        self.mlp3_ln1_1 = nn.LayerNorm([feature_base * 4, cells])
        # self.mlp3_bn1_2 = nn.BatchNorm1d(256)
        self.mlp3_ln1_2 = nn.LayerNorm([feature_base * 4, cells])
        self.mlp3_conv3 = torch.nn.Conv1d(feature_base * 4, feature_base * 2, 1)
        self.mlp3_conv4 = torch.nn.Conv1d(feature_base * 2, feature_base * 2, 1)
        # self.mlp3_bn2_1 = nn.BatchNorm1d(128)
        self.mlp3_ln2_1 = nn.LayerNorm([feature_base * 2, cells])
        # self.mlp3_bn2_2 = nn.BatchNorm1d(128)
        self.mlp3_ln2_2 = nn.LayerNorm([feature_base * 2, cells])
        # output
        self.output_conv = torch.nn.Conv1d(feature_base * 2, self.num_classes, 1)
...
if __name__ == '__main__':
    ...
    model = MeshSegNet(cells=14000, feature_base=16).to(device)
